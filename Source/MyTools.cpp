...
//Steps for reducing vertex set of the image
//Brute forced
//0. Initialize: TEMP = {}, x = 0, y = 0, count = 0
//1. for each i in N
//2. 	for each j in N
//3.		if Distance(i,j) < reductionSize && i != j
//4.			x += j.x; y += j.y; count++; TEMP << j
//5. 	if count > 3
//6. 		N << Point(x/|TEMP|, y/|TEMP|)
//7. 		N = TEMP XOR N
...
bool MyTools::isInside(Point pt, Mat img)
{
	...
	for(unsigned i = 0; i < 8; i++)
	{
		...
		int count = findIntersections(tempImg,img).size();
		if(count % 2 == 1)
			trueCount++;
		else
			falseCount++;
